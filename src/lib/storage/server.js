...
export async function serverDeleteFile(fileIdOrUrl) {
    ...
    try {
        ...
        // If it's not an ImageKit asset (e.g. data: or external google drive), skip safely
        if (fileIdOrUrl.startsWith('data:') || (fileIdOrUrl.startsWith('http') && !fileIdOrUrl.includes('imagekit.io') && !fileIdOrUrl.includes('ik.imagekit.io'))) {
            return { success: true, message: 'Not an ImageKit URL.' };
        }

        let fileId = fileIdOrUrl;

        // If a full URL was provided, extract the clean filename and delete from ImageKit
        if (fileIdOrUrl.startsWith('http://') || fileIdOrUrl.startsWith('https://')) {
            try {
                const parsedUrl = new URL(fileIdOrUrl);
                let searchPath = parsedUrl.pathname;
                
                // Strip imagekit endpoint prefix if present (e.g. /awsddit/...)
                const endpoint = process.env.IMAGEKIT_URL_ENDPOINT || process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT || '';
                if (endpoint) {
                    try {
                        const epPath = new URL(endpoint).pathname.replace(/\/$/, '');
                        if (epPath && searchPath.startsWith(epPath)) {
                            searchPath = searchPath.slice(epPath.length);
                        }
                    } catch {}
                }

                // Strip /tr:... transformations
                searchPath = searchPath.replace(/\/tr:[^/]+/g, '');
                ...
                if (searchName) {
                    ...
                    if (listResult && listResult.length > 0) {
                        for (const item of listResult) {
                            if (item.fileId) {
                                await ik.deleteFile(item.fileId).catch(() => {});
                            }
                        }
                        return { success: true, deletedCount: listResult.length };
                    } else {
        ...
        if (err.message && (err.message.includes('not found') || err.message.includes('404') || err.message.includes('does not exist'))) {
