...
export async function syncBadgeStudioToSupabase(data) {
    ...
    try {
        ...
        // Note: Badge Studio templates & fields are safely stored in onepass_badge_studio.
        // We do NOT inject template preview attendees into onepass_attendees to prevent resurrecting deleted event rosters.
...
/**
 * Sync template-specific attendees to Supabase (No-op to protect onepass_attendees isolation)
 */
export async function syncTemplateAttendeesToSupabase(templateId, attendeesList) {
    // Kept for backward-compatible interface; badge studio attendees remain scoped to onepass_badge_studio JSON.
    return;
}
