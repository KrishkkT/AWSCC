...
export async function syncTemplateAttendeesToSupabase(templateId, attendeesList) {
    ...
    try {
        // 1. Resolve a valid event_id from existing onepass_events
        let validEventId = null;
        const { data: events } = await supabase.from('onepass_events').select('id').limit(1);
        if (events && events.length > 0) {
            validEventId = events[0].id;
        } else {
            // Ensure default event exists in onepass_events to satisfy FK constraint
            const defaultEventId = 'evt_community_day_2026';
            await supabase.from('onepass_events').upsert({
                id: defaultEventId,
                name: 'AWS Students Community Day 2026',
                year: 2026,
                date: new Date().toISOString().split('T')[0],
                venue: 'DDU Campus, Nadiad',
                status: 'LIVE'
            }, { onConflict: 'id' });
            validEventId = defaultEventId;
        }

        const rows = attendeesList.map(a => ({
            id: a.id || `att_${Math.random().toString(36).substring(2, 10)}`,
            event_id: a.event_id || validEventId,
