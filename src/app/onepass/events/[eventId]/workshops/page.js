            ...
            <div className="p-6 bg-[#151c2e] border border-[#1a2540] rounded-3xl space-y-6 shadow-2xl text-center">
                ...
                <div className="flex justify-center">
                    <button
                        ...
                        disabled={!currentWk}
                        className={`flex items-center space-x-2 px-8 py-4 ${scannerOpen ? 'bg-red-600 hover:bg-red-700' : 'bg-purple-500 hover:bg-purple-400'} disabled:opacity-50 text-white font-extrabold text-sm rounded-2xl transition shadow-xl hover:scale-105`}
                    >
                        <Camera className="w-5 h-5 stroke-[2.5]" />
                        <span>{scannerOpen ? 'Close Camera Scanner' : 'Open Workshop Camera Scanner'}</span>
                    </button>
                </div>
                ...
                >
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="w-full bg-[#0C111D] border border-[#1a2540] rounded-xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-slate-500 font-mono outline-none focus:border-purple-500"
                        />
                    </div>
                    <button
                        type="submit"
                        disabled={!manualCode.trim() || evaluating || !currentWk}
                        className="px-5 py-2.5 bg-[#1a2540] hover:bg-purple-600 disabled:opacity-50 text-white rounded-xl text-xs font-semibold font-mono"
                    >
                        Verify
                    </button>
                </form>
                    ...
                    <div className="max-w-md mx-auto p-2 bg-[#0C111D] border border-[#1a2540] rounded-2xl space-y-1 max-h-56 overflow-y-auto text-left">
                        {searchResults.map((att) => (
                            <button
                                key={att.id}
                                onClick={() => {
                                    handleWorkshopScan(att.booking_id || att.qr_identifier || att.email || att.name);
                                    setSearchResults([]);
