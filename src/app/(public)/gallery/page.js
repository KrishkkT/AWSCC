...
export default function Gallery() {
    ...
    return (
        <div className="flex flex-col pt-32 bg-background min-h-screen relative overflow-hidden">
            ...
            <section className="py-20 relative z-10 text-center">
                <div className="container mx-auto px-6">
                    ...
                    <h1 className="text-5xl lg:text-7xl font-display font-bold mb-8 tracking-tight text-foreground">
            ...
            <section className="sticky top-16 z-40 backdrop-blur-xl border-y border-border py-6 bg-background/80">
                <div className="container mx-auto px-6">
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`px-6 py-2 rounded-xl text-xs font-bold transition-all border ${filter === cat
                                    ? "bg-brand-aws border-brand-aws text-white shadow-lg shadow-brand-aws/20"
                ...
                <div className="container mx-auto px-6">
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-32 space-y-4">
                            ...
                            <p className="text-muted-foreground font-medium animate-pulse">Synchronizing assets...</p>
                        </div>
                    ) : filteredPhotos.length === 0 ? (
                        <div className="text-center py-32 bg-secondary/30 rounded-[2.5rem] border border-border border-dashed">
                            <ImageIcon size={64} className="mx-auto text-muted-foreground/20 mb-6" />
                            <p className="text-muted-foreground font-medium">No snapshots found in this cluster.</p>
                        </div>
                    ) : (
                            ...
                            <AnimatePresence mode="popLayout">
                                {filteredPhotos.map((photo, i) => (
                                    <motion.div
                                        ...
                                        className="card-professional p-0 overflow-hidden cursor-zoom-in group border-border shadow-sm"
                                    ...
                                    >
                                        <div className="relative overflow-hidden aspect-auto">
                                            ...
                                            <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-8">
                                                ...
                                                <p className="text-foreground text-xl font-display font-bold tracking-tight">{photo.title}</p>
                                            </div>
                                        </div>
                                        <div className="p-4 bg-secondary/30 flex items-center justify-between border-t border-border">
                                            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{photo.event}</span>
                                        </div>
                                    </motion.div>
                                ))}
                            </AnimatePresence>
            ...
            <AnimatePresence>
                {selectedImage && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[100] bg-background/98 backdrop-blur-2xl flex items-center justify-center p-6"
                    ...
                    >
                        ...
                        >
                            <img
                                ...
                                className="max-w-full max-h-[75vh] object-contain rounded-2xl shadow-3xl border border-border"
                            />
                            <div className="mt-10 text-center">
                                <div className="text-brand-aws text-[10px] font-bold uppercase tracking-[0.3em] mb-4">{selectedImage.event}</div>
                                <h3 className="text-3xl md:text-5xl font-display font-bold text-foreground tracking-tight">{selectedImage.title}</h3>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
