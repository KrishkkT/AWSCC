-- ============================================================================
-- AWSCC MASTER PRODUCTION SCHEMA (CONSOLIDATED)
-- All-in-one database structure including RBAC, Events, community events, and certificate settings
-- ============================================================================
...
-- Function to check if a user is an admin/Leader/faculty/captain without causing recursion
create or replace function public.is_admin(user_id uuid)
returns boolean as $$
  select exists (
    select 1 from public.profiles
    where id = user_id
    and role in ('faculty', 'Leader', 'core', 'admin', 'captain')
    and is_active = true
  );
...
create table if not exists public.profiles (
  ...
  full_name text,
  avatar_url text,
  role text default 'member' check (role in ('faculty', 'Leader', 'core', 'member', 'admin', 'captain')),
...
-- I. COMMUNITY EVENTS (added in 117)
CREATE TABLE IF NOT EXISTS public.community_events (
    ...
    date TIMESTAMPTZ,
    venue TEXT,
    visibility_toggled BOOLEAN DEFAULT false,
    is_active BOOLEAN DEFAULT false,
    ...
    sponsors_data JSONB DEFAULT '[]'::jsonb,
    team_data JSONB DEFAULT '[]'::jsonb,
    ticket_data JSONB DEFAULT '{}'::jsonb,
    workshops_data JSONB DEFAULT '[]'::jsonb,
...
create policy "Events are viewable by everyone" on public.events for select using (status != 'draft' or public.is_admin(auth.uid()));
...
create policy "Public can register" on public.event_registrations for insert with check (true);
drop policy if exists "Admins can manage registrations" on public.event_registrations;
create policy "Admins can manage registrations" on public.event_registrations for all using (public.is_admin(auth.uid()));

-- CERTIFICATES POLICIES
drop policy if exists "Certificates viewable by everyone" on public.certificates;
create policy "Certificates viewable by everyone" on public.certificates for select using (true);
drop policy if exists "Admins can manage certificates" on public.certificates;
create policy "Admins can manage certificates" on public.certificates for all using (public.is_admin(auth.uid()));

-- AUDIT LOGS POLICIES
drop policy if exists "Admins can view logs" on public.audit_logs;
create policy "Admins can view logs" on public.audit_logs for select using (public.is_admin(auth.uid()));
drop policy if exists "System can log" on public.audit_logs;
create policy "System can log" on public.audit_logs for insert with check (true);

-- DYNAMIC CONTENT POLICIES
drop policy if exists "Content viewable by everyone" on public.resources;
create policy "Content viewable by everyone" on public.resources for select using (is_active = true);
drop policy if exists "Articles viewable by everyone" on public.knowledge_articles;
create policy "Articles viewable by everyone" on public.knowledge_articles for select using (is_published = true);
...
drop policy if exists "Admins can manage content" on public.resources;
create policy "Admins can manage content" on public.resources for all using (public.is_admin(auth.uid()));
drop policy if exists "Admins can manage articles" on public.knowledge_articles;
create policy "Admins can manage articles" on public.knowledge_articles for all using (public.is_admin(auth.uid()));
...
create policy "Admins can manage global settings" on public.global_settings for all using (public.is_admin(auth.uid()));

-- COMMUNITY EVENTS POLICIES
drop policy if exists "Community events are viewable by everyone" on public.community_events;
...
create policy "Admins can manage community events" on public.community_events for all using (public.is_admin(auth.uid()));
...
-- Insert placeholder community event for 2026
insert into public.community_events (year, title, is_active, visibility_toggled)
values (2026, 'AWS Students Community Day 2026', true, false)
on conflict (year) do nothing;

-- PROMOTE INITIAL ADMIN
update public.profiles set role = 'Leader', is_active = true where email = 'krishthakker508@gmail.com';
